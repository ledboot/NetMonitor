...
class MonitorClassVisitor extends ClassVisitor implements Opcodes {
    ...
    public MonitorClassVisitor(String path, String entry, boolean isJar, int api, ClassWriter cw) {
        super(api, cw)
        this.filePath = path
        this.entry = entry
        this.isJar = isJar
...
//        }
    ...
    private void collectAnno(String desc) {
        if (desc != null) {
            MonitorAnnotationCollector.ANNOTATION_CLASSES.each { anno ->
                if (!foundAnnos.contains(anno) && desc.contains(anno)) {
                    foundAnnos.add(anno)
                    MonitorAnnotationCollector.addNewAnno(anno, filePath, className, entry, isJar)
                }
            }
        }
    }
